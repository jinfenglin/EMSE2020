...
def train_with_neg_sampling(
    ...
    cache_file = "cached_twin_random_neg_sample_epoch_{}.dat".format(
        args.epochs_trained
    )
    if args.neg_sampling == "random":
        ...
        train_dataloader = train_examples.offline_neg_sampling_dataloader(
            model=model, batch_size=args.per_gpu_train_batch_size
        )
        ...
        raise Exception(
            "{} neg_sampling is not recognized...".format(args.neg_sampling)
        )
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if (
                ...
                and args.global_step % args.logging_steps == 0
            ):
                tb_data = {
            ...
            if (
                ...
                and args.global_step % args.save_steps == 1
            ):
                # step invoke checkpoint writing
                ckpt_output_dir = os.path.join(
                    args.output_dir, "step-ckp-{}".format(args.global_step)
                )
            ...
            if args.valid_step > 0 and args.global_step % args.valid_step == 1:
                validate(args, valid_examples, model, tb_writer)
        args.steps_trained_in_current_epoch += 1
        if 0 < args.max_steps < args.global_step:
            break

    if args.save_epoch > 0 and args.epochs_trained % args.save_epoch == 0:
        ckpt_output_dir = os.path.join(
            args.output_dir, "epoch-ckp-{}".format(args.epochs_trained)
        )
        save_check_point(model, ckpt_output_dir, args, optimizer, scheduler)

    if args.valid_epoch > 0 and args.epochs_trained % args.valid_epoch == 0:
        validate(args, valid_examples, model, tb_writer)


def validate(args, valid_examples, model, tb_writer):
    ...
    pk, best_f1, map = evaluate_retrival(
        model,
        valid_examples,
        args.per_gpu_eval_batch_size,
        "evaluation/{}/runtime_eval".format(args.neg_sampling),
...
def train(args, train_examples, valid_examples, model, train_iter_method):
    ...
    if args.max_steps > 0:
        ...
        args.num_train_epochs = (
            args.max_steps // (epoch_batch_num // args.gradient_accumulation_steps) + 1
        )
    else:
        t_total = (
            epoch_batch_num // args.gradient_accumulation_steps * args.num_train_epochs
        )
    ...
    if args.model_path and os.path.exists(args.model_path):
        ...
        logger.info(
            "  Continuing training from epoch {}, global step {}".format(
                args.epochs_trained, args.global_step
            )
        )
    else:
        logger.info("Start a new training")
    skip_n_steps_in_epoch = (
        args.steps_trained_in_current_epoch
...
def get_train_args():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="name of the crosslingual langauge model, including bert-base-cased, bert-base-multilingual-cased, distilbert-base-multilingual-cased",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The experiment name used as dir names for saving data",
    )
    ...
    valid_group.add_argument(
        ...
        type=int,
        default=-1,
        help="obtain validation accuracy every given epoch",
    )
    valid_group.add_argument(
        "--valid_step",
        type=int,
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="overwrite the cached data"
    )
    parser.add_argument(
    ...
    train_num_group = parser.add_mutually_exclusive_group()
    train_num_group.add_argument(
        "--num_train_epochs",
        default=3.0,
        type=float,
    ...
    save_group = parser.add_mutually_exclusive_group(required=True)
    ...
    save_group.add_argument(
        "--save_epoch", type=int, default=-1, help="Save checkpoint every X epoch"
    )
...
def init_train_env(args, tbert_type):
    ...
    if tbert_type == "twin" or tbert_type == "T":
        ...
        model = TBertSiamese(BertConfig(), multilingual_BERT)
    elif tbert_type == "single" or tbert_type == "S":
        model = TBertSingle(BertConfig(), multilingual_BERT)
    else:
...
def main():
    ...
    train_examples = load_examples(
        os.path.join(args.data_dir, "train"), model=model, num_limit=args.train_num
    )
    valid_examples = load_examples(
        os.path.join(args.data_dir, "valid"), model=model, num_limit=args.valid_num
    )
    train(
        args,
        train_examples,
        valid_examples,
        model,
        train_iter_method=train_with_neg_sampling,
    )
