...
from EMSE.utils import save_check_point, load_check_point, write_tensor_board, set_seed, evaluate_retrival, \
...
def train_with_neg_sampling(args, model, train_examples: Examples, valid_examples: Examples, optimizer,
    ...
    if args.neg_sampling == "random":
        ...
        raise Exception("{} neg_sampling is not recognized...".format(args.neg_sampling))
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if args.local_rank in [-1, 0] and args.save_steps > 0 and args.global_step % args.save_steps == 1:
                ...
                ckpt_output_dir = os.path.join(args.output_dir, "step-ckp-{}".format(args.global_step))
    ...
    if args.save_epoch > 0 and args.epochs_trained % args.save_epoch == 0:
        ckpt_output_dir = os.path.join(args.output_dir, "epoch-ckp-{}".format(args.epochs_trained))
        save_check_point(model, ckpt_output_dir, args, optimizer, scheduler)
...
def get_train_args():
    ...
    valid_group.add_argument('--valid_epoch', type=int, default=-1, help="obtain validation accuracy every given epoch")
    valid_group.add_argument("--valid_step", type=int, default=-1,
    ...
    train_num_group.add_argument("--num_train_epochs", default=3.0, type=float,
    ...
    save_group.add_argument("--save_steps", type=int, default=-1, help="Save checkpoint every X updates steps.")
    save_group.add_argument("--save_epoch", type=int, default=-1, help="Save checkpoint every X epoch")
...
def main():
    ...
    # formulat the train and test dataset here
